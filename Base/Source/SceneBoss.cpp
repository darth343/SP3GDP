...
bool SceneBoss::GetMonsterScaleUp()
{
	return monsterScaleUp;
}

void SceneBoss::SetMonsterScaleUp(bool set)
{
	this->monsterScaleUp = set;
}

float SceneBoss::GetBattleMonsterScaleX()
{
	return battleMonsterScale.x;
}

void SceneBoss::SetBattleMonsterScaleX(float x)
{
	this->battleMonsterScale.x = x;
}

float SceneBoss::GetBattleMonsterScaleY()
{
	return battleMonsterScale.y;
}

void SceneBoss::SetBattleMonsterScaleY(float y)
{
	this->battleMonsterScale.y = y;
}

float SceneBoss::GetBattleMonsterPosX()
{
	return battleMonsterPos.x;
}

float SceneBoss::GetBattleMonsterPosY()
{
	return battleMonsterPos.y;
}

void SceneBoss::SetBattleMonsterPosX(float x)
{
	this->battleMonsterPos.x = x;
}

void SceneBoss::SetBattleMonsterPosY(float y)
{
	this->battleMonsterPos.y = y;
}

void SceneBoss::SetGS(string set)
{
	if (set == "START_SCREEN")
		GS = START_SCREEN;
	else if (set == "TESTMAP")
...
void SceneBoss::CatchUpdate(double dt)
{
...
void SceneBoss::EnterBattleScene(Enemy* enemy)
{
...
void SceneBoss::PlayerUpdate(double dt)
{
	if (MS == PLAY)
	{
...
//For the mini game
void SceneBoss::TamagucciUpdate(double dt)
{
...
void SceneBoss::renderInventoryItems()
{
	for (int i = 0; i < SharedData::GetInstance()->inventory.EQinventory.size(); ++i)
	{
		if (SharedData::GetInstance()->inventory.EQinventory[i]->getName() == "UNDEFINED")
...
void SceneBoss::renderInventoryMenus()
{
...
void SceneBoss::RenderItemScreen()
{
	//std::ostringstream ss;
	//ss.str("");
	//ss.precision(5);
...
void SceneBoss::ItemScreenUpdate(double dt)
{
	if (Application::IsKeyPressed('I') && !SharedData::GetInstance()->IkeyPressed)
	{
		GS = TESTMAP;
...
void SceneBoss::RenderEquipScreen()
{
	std::ostringstream ss;
	ss.str("");
	ss.precision(5);
...
void SceneBoss::MapUpdate(double dt)
{
	if (MS == PLAY)
	{
		PlayerUpdate(dt);
	...
	for (int i = 0; i < m_goList.size(); i++)
	{
		if (m_goList[i]->active == true)
		{
			...
			if (m_goList[i]->type == GameObject::GO_ENEMY)
			{
				...
				Render2DMeshWScale(meshList[GEO_BOSS], false, m_goList[i]->scale.x, m_goList[i]->scale.y, m_goList[i]->position.x - theHero->GetMapOffset().x, m_goList[i]->position.y - theHero->GetMapOffset().y, temp->GetFlipStatus(),32);
			}
...
void SceneBoss::renderTamagotchiMenu()
{
	static float xpos = 0.f;
	static float ypos = 0.f;
...
void SceneBoss::renderTamagotchiGame()
{
	ostringstream ss;
	switch (tamagucci.getGameChoice())
	{
...
void SceneBoss::renderTamagotchiUI()
{
...
void SceneBoss::RenderTamagucci()
{
...
void SceneBoss::RenderBattleDialogue()
{
...
void SceneBoss::RenderBattleAnimation()
{
	if (SharedData::GetInstance()->BS_SlashRender)
...
void SceneBoss::RenderBattleHUD()
{
...
void SceneBoss::RenderBattleScene()
{
	RenderBackground(meshList[GEO_BATTLESCENE]);
	RenderMonster();
...
void SceneBoss::RenderTileMap(Mesh* mesh, CMap* map, Vector3 speed)
{
